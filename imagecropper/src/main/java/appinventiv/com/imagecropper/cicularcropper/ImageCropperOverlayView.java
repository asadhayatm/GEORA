 ...
 */
public class ImageCropperOverlayView extends View {
     ...
     */
    private final ImageCropperCropWindowHandler mImageCropperCropWindowHandler = new ImageCropperCropWindowHandler();
     ...
     */
    private ImageCropperWindowMoveHandler mMoveHandler;
    ...
    public ImageCropperOverlayView(Context context) {
        this(context, null);
    }

    public ImageCropperOverlayView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public RectF getCropWindowRect() {
        return mImageCropperCropWindowHandler.getRect();
    }
    ...
    public void setCropWindowRect(RectF rect) {
        mImageCropperCropWindowHandler.setRect(rect);
    }
    ...
    public void setBounds(float[] boundsPoints, int viewWidth, int viewHeight) {
        if (boundsPoints == null || !Arrays.equals(mBoundsPoints, boundsPoints)) {
            ...
            RectF cropRect = mImageCropperCropWindowHandler.getRect();
    ...
    public void setMinCropResultSize(int minCropResultWidth, int minCropResultHeight) {
        mImageCropperCropWindowHandler.setMinCropResultSize(minCropResultWidth, minCropResultHeight);
    }
    ...
    public void setMaxCropResultSize(int maxCropResultWidth, int maxCropResultHeight) {
        mImageCropperCropWindowHandler.setMaxCropResultSize(maxCropResultWidth, maxCropResultHeight);
    }
    ...
    public void setCropWindowLimits(float maxWidth, float maxHeight, float scaleFactorWidth, float scaleFactorHeight) {
        mImageCropperCropWindowHandler.setCropWindowLimits(maxWidth, maxHeight, scaleFactorWidth, scaleFactorHeight);
    }
     ...
     */
    public void setInitialAttributeValues(ImageCropperOptions options) {

        mImageCropperCropWindowHandler.setInitialAttributeValues(options);
    ...
    private void initCropWindow() {
        ...
        if (mInitialCropWindowRect.width() > 0 && mInitialCropWindowRect.height() > 0) {
            ...
            rect.left = leftLimit + mInitialCropWindowRect.left / mImageCropperCropWindowHandler.getScaleFactorWidth();
            rect.top = topLimit + mInitialCropWindowRect.top / mImageCropperCropWindowHandler.getScaleFactorHeight();
            rect.right = rect.left + mInitialCropWindowRect.width() / mImageCropperCropWindowHandler.getScaleFactorWidth();
            rect.bottom = rect.top + mInitialCropWindowRect.height() / mImageCropperCropWindowHandler.getScaleFactorHeight();
            ...
            if (bitmapAspectRatio > mTargetAspectRatio) {
                ...
                float cropWidth = Math.max(mImageCropperCropWindowHandler.getMinCropWidth(), rect.height() * mTargetAspectRatio);
                ...
                // Limits the aspect ratio to no less than 40 wide or 40 tall
                float cropHeight = Math.max(mImageCropperCropWindowHandler.getMinCropHeight(), rect.width() / mTargetAspectRatio);
    ...
    private void fixCropWindowRectByRules(RectF rect) {
        if (rect.width() < mImageCropperCropWindowHandler.getMinCropWidth()) {
            float adj = (mImageCropperCropWindowHandler.getMinCropWidth() - rect.width()) / 2;
            rect.left -= adj;
            rect.right += adj;
        }
        if (rect.height() < mImageCropperCropWindowHandler.getMinCropHeight()) {
            float adj = (mImageCropperCropWindowHandler.getMinCropHeight() - rect.height()) / 2;
            rect.top -= adj;
            rect.bottom += adj;
        }
        if (rect.width() > mImageCropperCropWindowHandler.getMaxCropWidth()) {
            float adj = (rect.width() - mImageCropperCropWindowHandler.getMaxCropWidth()) / 2;
            rect.left += adj;
            rect.right -= adj;
        }
        if (rect.height() > mImageCropperCropWindowHandler.getMaxCropHeight()) {
            float adj = (rect.height() - mImageCropperCropWindowHandler.getMaxCropHeight()) / 2;
            rect.top += adj;
            rect.bottom -= adj;
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mImageCropperCropWindowHandler.showGuidelines()) {
    ...
    private void drawBackground(Canvas canvas) {
        ...
        RectF rect = mImageCropperCropWindowHandler.getRect();
    ...
    private void onActionDown(float x, float y) {
        mMoveHandler = mImageCropperCropWindowHandler.getMoveHandler(x, y, mTouchRadius, mCropShape);
    ...
    private class ScaleListener extends ScaleGestureDetector.SimpleOnScaleGestureListener {
        ...
        public boolean onScale(ScaleGestureDetector detector) {
            ...
            if (newLeft < newRight &&
                    newTop <= newBottom &&
                    newLeft >= 0 &&
                    newRight <= mImageCropperCropWindowHandler.getMaxCropWidth() &&
                    newTop >= 0 &&
                    newBottom <= mImageCropperCropWindowHandler.getMaxCropHeight()) {
