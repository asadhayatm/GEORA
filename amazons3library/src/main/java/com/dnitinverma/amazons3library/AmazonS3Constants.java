package com.dnitinverma.amazons3library;

import com.amazonaws.regions.Regions;

/**
 * Created by appinventiv on 11/7/18.
 */

public class AmazonS3Constants {
//    public static final String AMAZON_POOLID = "us-east-1:b1f250f2-66a7-4d07-96e9-01817149a439";
//    public static final String BUCKET = "appinventiv-development";
//    public static final String AMAZON_SERVER_URL = "https://appinventiv-development.s3.amazonaws.com/";
//    public static final String END_POINT = "s3.amazonaws.com";
//    public static final Regions REGIONS = Regions.US_EAST_1;

    public static final String AMAZON_POOLID = "us-east-1:a7a1d9a3-a3ee-4244-990e-09916294dd51";
    public static final String BUCKET = "geora";
    public static final String AMAZON_SERVER_URL = "https://geora.s3.amazonaws.com/";
    public static final String END_POINT = "s3.amazonaws.com";
    public static final Regions REGIONS = Regions.US_EAST_1;
}
